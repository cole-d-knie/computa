...
This same dependency set applies when any `computa-swarm-verify-*` subskill is invoked directly.
...
1. `computa-swarm-verify-setup`
2. `computa-speak`
3. `computa-swarm-verify-investigate`
4. `computa-swarm-verify-tdd-qa`
...
Also read `references/master-computa-swarm-verification-prompt.md` when present. It is the parity source. If a detail is present there and absent here, follow the reference unless the user explicitly overrides it.
...
1. Phase 0: use `computa-swarm-verify-setup` to create artifacts, preserve the raw request, split phases/tasks/subtasks, initialize ledgers, and append `session_started` to `activity-log.csv`.
...
3. Phase 1: use `computa-swarm-verify-investigate` to run an orientation audit of the task and working codebase before implementation. Create a living `maps/` directory that later phases can reference.
4. Phase 2: continue `computa-swarm-verify-investigate` to establish baseline truth, reproduce the issue or prove the gap, identify expected behavior, and create failing evidence.
5. Implementation phases: use `computa-swarm-verify-tdd-qa` for test-first changes, smallest fixes, edge cases, unit/integration/smoke/runtime QA, and Playwright verification.
...
7. After every task and phase: use `computa-swarm-verify-swarms` for adversarial review followed by judge/verifier review. Do not implement adversarial recommendations unless the matching judge/verifier approves.
8. Closeout: use `computa-swarm-verify-closeout` to reconcile ledgers, append final `session_completed` or `session_blocked` status to `activity-log.csv`, create final reports, compare against `user-task.md`, record new fixed/unfixed issues, verify docs status, verify evidence, and state remaining gaps.
