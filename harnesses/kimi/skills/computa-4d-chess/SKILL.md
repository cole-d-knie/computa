...
1. `computa-init`
2. `computa-speak`
3. `computa-resume` when recovering prior 4D work
4. `computa-secrets-needed`
5. `computa-4d-chess-architect`
6. `computa-4d-chess-build`
7. `computa-4d-chess-execute`
8. `security-audit`

During execution, each Super-Phase must invoke `computa-make-no-mistakes`.

Also use or emulate the Computa dependency set when available: `caveman`, `using-superpowers`, `writing-plans`, `executing-plans`, `test-driven-development`, `systematic-debugging` or `investigate`, native agent swarm/delegation, `requesting-code-review`, `verification-before-completion`, `playwright`, and `context7`.
...
3. If `docs/architecture/` exists, read it first for orientation before architecture decisions, then verify claims against code during the codebase audit.
4. If a repo/codebase exists, run a codebase audit before architecture decisions. Do not design Super-Phases from chat context or desired architecture alone.
5. Run `computa-4d-chess-architect` to audit, map, spec, decide, and produce the strategic architecture.
6. Run `computa-4d-chess-build` to create the Super-Phase architecture and artifact tree.
7. Review each created Super-Phase with an adversarial reviewer, then a judge/verifier.
8. Review the full Super-Phase plan with an adversarial reviewer, then a judge/verifier.
9. Run `computa-4d-chess-execute` to execute each approved Super-Phase by invoking `/computa-make-no-mistakes`.
10. After every Super-Phase execution, run Super-Phase-level adversarial review, then judge/verifier review.
11. Treat `SP-999-post-run-security-audit` as the mandatory final Super-Phase. It must run `/computa-make-no-mistakes` with a task that invokes `/security-audit` in 4D final Super-Phase implementation mode.
12. Finish with 4D closeout reports that synthesize all Super-Phase Computa closeouts and the security audit into one low-context handoff.
